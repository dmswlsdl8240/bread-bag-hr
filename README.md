...
3. **로컬 개발**: `config.example.js`를 같은 폴더에 `config.js`로 복사한 뒤 실제 Supabase URL / anon
   publishable key / EmailJS 값을 채워 넣기. **`config.js`는 `.gitignore`에 등록되어 있어
   git에는 올라가지 않으므로, 리포지토리 소스에는 실제 키가 남지 않습니다.**
4. **Vercel 배포**: `config.js`는 git에 올라가지 않으므로 Vercel이 빌드 시점에 환경변수로
   생성합니다 (`generate-config.js` + `vercel.json`의 `buildCommand`).
   Vercel 프로젝트 설정 → Environment Variables에 아래 값을 등록하세요
   (Production/Preview/Development 모두):
   - `SUPABASE_URL`, `SUPABASE_KEY` (필수)
   - `DESIGNER_EMAIL`, `EMAILJS_SERVICE`, `EMAILJS_TEMPLATE`, `EMAILJS_PUBLIC_KEY` (선택, EmailJS 알림 쓸 때만)
   - 값이 바뀌면 Vercel 대시보드에서 값 수정 후 재배포(redeploy)하면 반영됩니다.
5. (선택) 관리자 개인 기기에서 다른 Supabase 프로젝트를 쓰고 싶다면 각 화면의
...
6. 두 파일을 같은 도메인에 배포하면 설정값(`hr_supabase_cfg`)이 공유됨
